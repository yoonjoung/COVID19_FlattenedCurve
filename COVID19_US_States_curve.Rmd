...
dtaregion<-read_excel("C:/Users/YoonJoung Choi/Dropbox/0 Project/COVID19_US/DataUSDA/StatesByCensusRegionAndDivision.xlsx", sheet="Sheet1") %>%
...
(Published on November 1, 2020. Last updated on `r date`) 
...
####__Northeast__
_Note: The x-axis is date since March. The y-axis is 7-day rolling average of daily new cases per 100,000 population. The gray line is the US national data. The light gray box represents the number of daily new cases 10 or lower per 100,000._  
...
dtafig<-dtacurve%>%filter(region=="Northeast")
...
dtafig<-dtacurve%>%filter(region=="South" & division=="South Atlantic")
maxy<-max(dtafig$newcasessmoothpp)
...
_Hover over the figures to see the underlying data._
```{r plotSouthCentral, results="asis", fig.align="left", out.width="800px", out.height="650px"}
...
_Hover over the figures to see the underlying data._
```{r plotMidwest, results="asis", fig.align="left", out.width="800px", out.height="650px"}

dtafig<-dtacurve%>%filter(region=="Midwest" & (state!="Wisconsin" & state!="North Dakota" & state!="South Dakota"))
maxy<-max(dtafig$newcasessmoothpp)
...
```{r plotMidwestHigh, results="asis", fig.align="left", out.width="800px", out.height="250px"}
dtafig<-dtacurve%>%filter(region=="Midwest" & (state=="Wisconsin" | state=="North Dakota" | state=="South Dakota"))
maxy<-max(dtafig$newcasessmoothpp)
...
dtafig<-dtacurve%>%filter(region=="West")
maxy<-max(dtafig$newcasessmoothpp)
...
```{r group}
#FOUR groups
temp<-dtacurve%>%filter(peakcasespp>=0  & peakcasespp<10)%>%filter(latest==TRUE)
    length(unique(temp$state))    
    table(temp$state)
    ngroup1<-length(unique(temp$state))
    listgroup1<-unique(temp$state)

temp<-dtacurve%>%filter(peakcasespp>=10 & peakcasespp<30)%>%filter(latest==TRUE)
    length(unique(temp$state))    
    table(temp$state)
    ngroup2<-length(unique(temp$state))
    listgroup2<-unique(temp$state)
    
temp<-dtacurve%>%filter(peakcasespp>=30 & peakcasespp<60)%>%filter(latest==TRUE)
    length(unique(temp$state))    
    table(temp$state)
    ngroup3<-length(unique(temp$state))
    listgroup3<-unique(temp$state)    
    
temp<-dtacurve%>%filter(peakcasespp>=60)%>%filter(latest==TRUE)
    length(unique(temp$state))    
    table(temp$state)
    ngroup4<-length(unique(temp$state))
    listgroup4<-unique(temp$state)        
```
...
panel <- . %>% 
    plot_ly(x = ~date)%>% 
    add_trace(
        y = ~levellow, type = 'scatter', mode = 'lines',
        line= list(color = "#EEEEEE") ) %>%    
    add_trace(
        y = ~levelhigh, type = 'scatter', mode = 'lines',
...
```{r plotSTATES1, fig.align="left", out.width="800px", out.height="250px"}

dtafig<-dtacurve%>%filter(peakcasespp>=0  & peakcasespp<10)%>%
    mutate(levellow=0, levelhigh=10)
...
```{r plotSTATES2, fig.align="left", out.width="800px", out.height="650px"}

dtafig<-dtacurve%>%filter(peakcasespp>=10  & peakcasespp<30)%>%
    mutate(levellow=10, levelhigh=30)
...
```{r plotSTATES3, fig.align="left", out.width="800px", out.height="1650px"}

dtafig<-dtacurve%>%filter(peakcasespp>=30  & peakcasespp<60)%>%
    mutate(levellow=30, levelhigh=60)
...
```{r plotSTATES4, fig.align="left", out.width="800px", out.height="450px"}

dtafig<-dtacurve%>%filter(peakcasespp>=60)%>%
    mutate(levellow=60, 
           levelhigh=140)
