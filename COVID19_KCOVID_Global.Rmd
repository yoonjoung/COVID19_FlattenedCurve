...
(Last updated on `r date`. A version as of April 26th was published at [Medium](https://medium.com/@yj.choi.isquared/flattening-covid-19-curve-in-south-korea-and-comparison-among-oecd-countries-singapore-and-taiwan-ae211a5645c9))
...
plot_ly(dtafig, x=~date, 
    ...
    add_segments(x = as.Date("2020-02-23"), xend = as.Date("2020-05-02"), 
...
Among OECD members, `r nlikeSK` other countries also have entered the relatively stable phase, although the first wave started later. _Holding beginning of the first wave on day 0_, the following figure presents the length and height of the peak. Except in Greece, the peak height is taller - slightly or substantially (Iceland and Spain). And, the length of pre and post peak phases is longer. 

It is notable that two countries with a substantially high peak have not entered this phase.  
* __Iceland is the first country that entered the relatively stable phase, among those with a very high peak__ (see the following section), but it took over 50 days to pass the peak phases.   
* __In Spain, the number of new cases indeed had come down to the start level after 52 days, but then it quickly rebounded__.  

Spain's example reinforces that, even among these countries, it is important to keep the number of new infections low without any rebound.

Note: The x-axis is days from the first day of phase 1. The y-axis is daily new cases per 100,000 population (necessary for comparing big and small countries).
...
```{r plotSKlike, fig.align="left", out.width="800px"}
...
```{r plotSKlike_thumbnail, results="asis", fig.align="left", out.width="800px", out.height="500px"}
...
* Some would reach the stable phase soon if the current pace continues (e.g., Germany and Luxembourg).  
* Some have had rebounds. After a tentative peak, the number of new infections decreased substantially, but then increased again. See Finland.  
* Some reached or passed the peak only recently. After the peak, new cases decreased immediately and consistently in some countries (e.g., Austria), while some countries have a wide peak (e.g., Belgium and United States).  
* Some have entered the first wave, and the curve is going up slowly (e.g., Mexico and Poland). The level is low, but there is no clear sign of decrease.

```{r plotSKunlikeMedium, fig.align="left", out.width="600px", out.height="300px"}
...
dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)%>%filter(peakcasespp<4)
...
```{r plotSKunlike2, results="asis", fig.align="left", out.width="800px", out.height="650px"}
...
panel <- . %>% 
  ...
  layout(
    showlegend = FALSE,
    title = c(""),
    xaxis=list(range=c(0,60), title=""), 
    yaxis=list(range=c(0,15), title="") 
  )

dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)%>%filter(peakcasespp>=4 & peakcasespp<13)
...
dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)%>%filter(peakcasespp>=13)
...
To summarize, the first figure below shows the timeline of the first wave and its phases among OECD countries. __South Korea had the first wave earlier than any other countries__ (red dots). __It also has one of the shortest length of the peak__ (between red and yellow dots). Again, the peak length is still to be determined in many countries (with no yellow dot below). 
