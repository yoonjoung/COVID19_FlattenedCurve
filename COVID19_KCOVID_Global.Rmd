...
dta<-dta%>%arrange(country, date)%>%
    ...
    mutate(
        newcasessmooth =c(NA,NA,rollmean(newcases, 5),NA,NA), 
        newdeathssmooth=c(NA,NA,rollmean(newdeaths, 5),NA,NA), 
...
dtacurve<-dtacurve%>%filter(incidence>1)
    length(unique(dtacurve$country))
...
dtacurve<-dtacurve%>%
    ...
    ungroup()%>%
    select(country, oecd, date, day, latest, cases, deaths, incidence, cfr, mortality, starts_with("new"), starts_with("start"), starts_with("peak"), starts_with("end"), -endcases)
...
We have learned how South Korea has implemented a unique strategy - universally across the country and consistently from the outset of the epidemic. Then, how does South Korean _flattened curve_ look like, compared to other countries? __This report compares metrics of the epidemic curve in South Korea and other high-resource settings, specifically OECD member countries__. In addition, Taiwan and Singapore are included considering their exemplary handling of the epidemic.     
...
plot_ly(dtafig, x=~day, y=~Y_Australia, name = "Australia", 
               type = 'scatter', mode='lines', line = list(color = c("#c7e9c0"))  )%>%
    add_trace(y=~Y_Greece, name="Greece", 
                  line = list(color = c("#c6dbef")) )%>%
    add_trace(y=~Y_NZ, name="New Zealand", 
                  line = list(color = c("#d9d9d9")) )%>%
    add_trace(y=~Y_Slovenia, name="Slovenia", 
                  line = list(color = c("#dadaeb")) )%>%
...
* Some have had __multiple small peaks__. At each time, the number of new infection decreased substantially but then increased again (e.g., Denmark, Estonia, and Sweden).
...
```{r plotSKunlike, results="asis", fig.align="left", out.width="800px", out.height="600px"}
dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)

#table(dtafig$peakcasespp)

dtafig1<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)%>%filter(peakcasespp<7)
dtafig2<-dtacurve%>%filter(oecd==TRUE)%>%filter(is.na(enddate)==TRUE)%>%filter(peakcasespp>=7)

fig<-plot_ly(dtafig, x=~day, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', color=~country)%>%
    layout(showlegend=TRUE,
           xaxis = list(title = "Day since start of the first wave", 
                        tickfont = list(size=10)),
           yaxis = list(title = "Daily new confirmed cases per 100,000 population") ) 

fig1<-plot_ly(dtafig1, x=~day, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', color=~country)%>%
    layout(showlegend=TRUE,
           xaxis = list(title = "Day since start of the first wave", 
                        tickfont = list(size=10)),
           yaxis = list(title = "Daily new confirmed cases per 100,000 population") ) 

fig2<-plot_ly(dtafig2, x=~day, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', color=~country)%>%
    layout(showlegend=TRUE,
           xaxis = list(title = "Day since start of the first wave", 
                        tickfont = list(size=10)),
           yaxis = list(title = "Daily new confirmed cases per 100,000 population") ) 

subplot(fig1, fig2, nrows=2, shareY = FALSE, shareX = TRUE)%>%
    layout(showlegend=TRUE,
           xaxis = list(title = "Day since start of the first wave", 
                        tickfont = list(size=10)),
           yaxis = list(title = "Daily new confirmed cases per 100,000 population") )
...
```{r plotSKcompare, results="asis", fig.align="left", out.width="800px"}

figSK<-dtacurve%>%filter(country=="South Korea")%>%
    plot_ly()%>%
    add_trace(dtafig, x=~date, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', name="South Korea")    

figTaiwan<-dtacurve%>%filter(country=="Taiwan")%>%
    plot_ly()%>%
    add_trace(dtafig, x=~date, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', name="Taiwan") 

figSingapore<-dtacurve%>%filter(country=="Singapore")%>%
    plot_ly()%>%
    add_trace(dtafig, x=~date, y=~newcasessmoothpp, type = 'scatter',mode = 'lines', name="Singapore") 


subplot(figTaiwan, figSingapore, figSK, 
        nrows = 1, shareX = TRUE, shareY = TRUE)%>%
    layout(
            title = c("Trend of daily new confirmed cases per 100,000 population since the first wave"),
        xaxis = list(title = "", tickfont = list(size=10)),  
        yaxis = list(title = "New confirmed cases per 100,000 population"), 
        legend = list(orientation = 'v',x = 0.1, y = 0.9, legendfont = list(size=8))
        )

```

####__3. Putting all together__
