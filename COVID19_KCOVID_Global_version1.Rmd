...
__(Note: as of August 19th, 2020, the numnber of new cases (7-day rolling average) has surpassed the stable phase threshold in South Koare. So do several other countries that used to be in the stable phase.)__
...
```{r plotSKrate_original, fig.align="left", out.width="800px"}
...
shapelist<-list(
            ...
            list(
                ...
                x1=as.character("2020-03-11"),xref = "x",
                y0=0,y1=300, yref = "y2" ),
            list(
                type = "rect",  fillcolor = "#fed976", 
                line = list(color = "#fed976"), opacity = 0.2,
                x0=as.character("2020-03-11"), 
...
Among OECD members, currently `r nlikeSK` other countries also have entered the relatively stable phase, although the first wave started later. _Holding beginning of the first wave on day 0_, the following figure presents the length and height of the peak. 
...
```{r plotSKlike, eval=FALSE, fig.align="left", out.width="200px"}
...
```{r plotSKlike_thumbnail, results="asis", fig.align="left", out.width="800px", out.height="200px"}
...
```{r plotSKunlikeMedium, fig.align="left", out.width="800px", out.height="100px"}
...
```{r plotSKunlike1, results="asis", fig.align="left", out.width="300px", out.height="200px"}
...
panel <- . %>% 
  ...
  layout(
    ...
    yaxis=list(range=c(0,50), title="") 
  )

dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(latestcasespp>startcasespp)%>%filter(peakcasespp>=4 & peakcasespp<50)
...
```{r plotSKunlike3, results="asis", fig.align="left", out.width="800px", out.height="600px"}
...
panel <- . %>% 
  ...
  layout(
    ...
    title = c(""),
    xaxis=list(range=c(0, maxday), title=""), 
    yaxis=list(title=c(0, 30),"") 
  )

dtafig<-dtacurve%>%filter(oecd==TRUE)%>%filter(latestcasespp>startcasespp)%>%filter(peakcasespp>=50)
